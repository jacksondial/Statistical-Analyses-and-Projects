...
covid_top_cities<- covid %>% arrange(desc(confirmed)) %>% top_n(50) %>% select(confirmed, everything())
```
...
#Create ranking of top cities
covid_top_cities<- covid %>% arrange(desc(confirmed)) %>% top_n(50)  %>% mutate(rank = 1:50) %>% select(confirmed,rank, everything())
...
First the set-up
```{r}
pal<-colorNumeric(palette = c("yellow", "red"), domain = (covid_top_cities$confirmed))
...
leaflet() %>% addTiles() %>% 
  addPolygons(data = counties, fill = FALSE, weight = 1, color = "blue") %>%
  addPolygons(data = state48, fill = FALSE, weight = 2, color = "black") %>% 
  addMarkers(data = covid,
             clusterOptions =  markerClusterOptions(),
             lng = ~Long_,
             lat = ~Lat,
